...
export const consultarGemini = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<GeminiResponse> => {
    ...
    const callGemini = async (model: string): Promise<GeminiResponse> => {
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${process.env["GEMINI_API_KEY"]}`,
        {
          ...
          body: JSON.stringify(buildGeminiBody(prompt, { temperature: 0.1 })),
        },
      );
    ...
    // 1️⃣ Intento con alias dinámico gemini-flash-latest
    let result = await callGemini("gemini-flash-latest");

    // 2️⃣ Fallback automático si no hubo output
    if (!result.output) {
